...
import { BillItem, Tenant } from "../services/types";
...
export const GenerateInvoicePage: React.FC<GenerateInvoiceProps> = ({
  setView,
}) => {
  ...
  const [invoiceType, setInvoiceType] = useState<"general" | "specific">(
    "general",
  );
  ...
  useEffect(() => {
    const fetchData = async () => {
      const [billData, tenantData] = await Promise.all([
        fetchBills(),
        db.getAllTenants(),
      ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!paymentStart || !paymentEnd)
      return alert("Payment start and end dates required");
    if (invoiceType === "specific" && !tenantId)
            ...
            >
              ...
              {bills.map((b) => (
                <option key={b.id} value={b.id}>
                  {b.name} | {b.billing_cycle} | {b.invoice_type} | ₦
