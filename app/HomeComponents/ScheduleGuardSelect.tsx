...
interface UnifiedGuardComboDropdownProps {
  guards: any[];
...
export const UnifiedGuardComboDropdown = ({
  guards,
  selectedGuardIds,
  onChange,
}: UnifiedGuardComboDropdownProps) => {
  const [isOpen, setIsOpen] = useState(false);

  const toggleGuardAssignment = (guardId: string) => {
    const next = selectedGuardIds.includes(guardId)
      ? selectedGuardIds.filter((id) => id !== guardId)
      : [...selectedGuardIds, guardId];
    onChange(next);
  };
      ...
      >
        <span className="truncate">
          {selectedGuardIds.length === 0
            ? "Select guards..."
            : `${selectedGuardIds.length} Guard${selectedGuardIds.length > 1 ? "s" : ""} Selected`}
        </span>
        <ChevronDown size={14} className="text-slate-400" />
      </button>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="w-full max-w-sm bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden flex flex-col">
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
              <h3 className="text-xs font-bold text-slate-800 uppercase tracking-wide">
                Assign Guards Combination throughout
              </h3>
              <button
                type="button"
                onClick={() => setIsOpen(false)}
            ...
            <div className="h-64 overflow-y-auto p-3 space-y-1">
              {guards.length === 0 ? (
                ...
                guards.map((g: any) => {
                  const isAssigned = selectedGuardIds.includes(g.id);
                  return (
                    <label
                      key={g.id}
                      className="flex items-center gap-3 px-3 py-2 hover:bg-slate-50 rounded-xl cursor-pointer text-xs font-medium text-slate-700 transition-colors border border-transparent hover:border-slate-100"
