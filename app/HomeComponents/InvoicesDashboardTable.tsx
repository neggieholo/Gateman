...
import { InvoiceWithTenant, EstateInvoice } from "../services/types";
...
function InvoiceTable<T extends InvoiceWithTenant | EstateInvoice>({
  invoices,
  type,
}: InvoiceTableProps<T>) {
  return (
    <div className="bg-white rounded-4xl p-6 shadow-sm border overflow-hidden">
      <div className="p-6 border-b flex items-center justify-between">
        <h2 className="font-bold text-lg text-slate-900">
          {type === "tenant"
            ? "Recent Residents Invoices"
            : "Recent Estate Invoices"}
        </h2>
                ...
                if (type === "tenant") {
                  ...
                  return (
                    <tr key={tenantInv.id}>
                      ...
                      <td>
                        {tenantInv.tenant.block}-{tenantInv.tenant.unit}
                      </td>
                      <td>
                        ₦
                        {tenantInv.total_amount.toLocaleString(undefined, {
                          minimumFractionDigits: 2,
                        })}
                      </td>
                      <td>
                        {new Date(tenantInv.created_at).toLocaleDateString()}
                      </td>
                  ...
                  return (
                    <tr key={estateInv.id}>
                      ...
                      <td>
                        ₦
                        {estateInv.total_amount.toLocaleString(undefined, {
                          minimumFractionDigits: 2,
                        })}
                      </td>
                      ...
                      <td>
                        {estateInv.period_start
                          ? new Date(
                              estateInv.period_start,
                            ).toLocaleDateString()
                          : "-"}{" "}
                        -{" "}
                        {estateInv.period_end
                          ? new Date(estateInv.period_end).toLocaleDateString()
                          : "-"}
                      </td>
                    </tr>
                  );
