...
export default function InvoicesPage() {
  ...
  const [recentEstateInvoices, setRecentEstateInvoices] = useState<
    EstateInvoice[]
  >([]);
  ...
  return (
    <>
      {currentView === "dashboard" && (
        <div className="space-y-8 pb-24 md:pb-8 flex flex-col">
          ...
          <div className="w-full flex justify-end gap-4 mt-6">
            <button
              className="px-6 py-2 bg-emerald-600 text-white rounded-xl font-semibold shadow-lg hover:bg-emerald-700 flex items-center gap-2"
              onClick={() => {
                setCurrentView("generateInvoice");
              }}
            >
      ...
      {currentView === "generateInvoice" && (
        <GenerateInvoicePage setView={setCurrentView} />
      )}
    </>
  );
}
